...
const ViewMediasFilesLinks = () => {
    ...
    React.useEffect(() => {
        if (selectedRoomid) {
            axios
                .get(
                    `${process.env.REACT_APP_CHAT_SERVER_URL}api/chat/message/attachment/${selectedRoomid}`
                )
                .then((res) => {
                    let copyAttachments = [];
                    res.data.messagesHaveAttachment.length > 0 &&
                        res.data.messagesHaveAttachment.forEach((message) => {
                            copyAttachments = [
                                ...copyAttachments,
