...
const SearchBar = ({ target, placeholder, autoFocus }) => {
    ...
    const handleSearch = async (value) => {
        ...
        if (target === 'profile') {
            const results = await axios.get(
                `${process.env.REACT_APP_AUTH_SERVER_URL}api/profile/search?string=${value}`
            );

            if (results.status === 200) {
                let resultsLater = results.data.profiles;
    ...
    const debounceSearchResult = React.useCallback(
        debounce((value) => handleSearch(value), debounceTime),
        []
    );
    const handleSearchInput = (e) => {
        setIsLoading(true);
        setString(e.target.value);
        debounceSearchResult(e.target.value);
    };
    React.useEffect(() => {
        if (string === '') {
            setProfiles([]);
