...
import { uploadToCloudinary, updateProfile } from '~/tools/callApi';
...
const Profile = () => {
    ...
    const handleUpdateProfile = async () => {
        ...
        if (updatedProfile.status === 'success') {
            dispatch(
                authSlice.actions.changeProfile(updatedProfile.updatedProfile)
            );
            setForm(false);
            setIsLoading(false);
        } else {
        ...
        const getProfile = async () => {
            try {
                const { data } = await axios.get(
                    `${process.env.REACT_APP_AUTH_SERVER_URL}api/profile/${profileid}`
                );
                    ...
                    <div className={clsx('profile-body', { form })}>
                        <div className='profile-body-details'>
                            ...
                            <div className='profile-body-item profile-date-of-birth'>
                                ...
                                &nbsp;
                                {otherProfile
                                    ? moment(otherProfile.dateofbirth).format(
                                          'DD/MM/yyyy'
                                      ) ||
                                      `[
