...
import { authSlice, componentSlice } from '../redux/slices';
import { useNavigate } from 'react-router-dom';
import { BsEye, BsEyeSlash } from 'react-icons/bs';
import { validateFormInput, removeValidateError } from './validateFormInput';
import { register } from '~/tools/callApi';
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (user) {
                ...
                dispatch(componentSlice.actions.setAllInMessage());
                return navigate('/', { replace: true });
            }
        } catch (error) {
            // dispatch(authSlice.actions.unLoading());
            console.log(error);
            // if (currentLanguage.languageCode === 'en') {
            //     return setError(user.message);
            // } else if (currentLanguage.languageCode === 'vn') {
            //     return setError(user.messagevn);
            // }
        }
    };
