    ...
    React.useEffect(() => {
        const loadAccount = async () => {
            const { data } = await axios.get(
                `${process.env.REACT_APP_AUTH_SERVER_URL}api/auth/me`
            );
            ...
            {!isLoading && (
                <>
                    <div className='account-modal-body-item'>
                        <MdOutlineAdminPanelSettings />{' '}
                        {currentLanguage.username} : {myAccount.username}
                    </div>
                    <div className='account-modal-body-item'>
                        <MdOutlineVerifiedUser /> {currentLanguage.role} :{' '}
                        {myAccount.role}
                    </div>
                    <div className='account-modal-body-item'>
                        <MdOutlineEmail /> {currentLanguage.email} :{' '}
                        {myAccount.email}
                    </div>
                    <div className='account-modal-body-item'>
