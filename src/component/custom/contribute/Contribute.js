...
import { uploadFileInstance } from '~/tools/axios';
...
const Contribute = () => {
    ...
    const handleSendContribute = async (e) => {
        ...
        e.preventDefault();
        try {
            const { data } = await uploadFileInstance.post(
                `${process.env.REACT_APP_UPLOAD_SERVER_URL}api/contribute`,
                formData,
                {
                    headers: {
