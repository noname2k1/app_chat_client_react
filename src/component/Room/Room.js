...
import './Room.scss';
import { roomInstance } from '~/tools/axios';
...
const Room = ({ closeBox }) => {
    ...
    const handleButtonClick = async (e) => {
        ...
        if (tabName === 'createRoom') {
            try {
                const { data } = await roomInstance.post(
                    `${process.env.REACT_APP_CHAT_SERVER_URL}api/chat/room?mode=room`,
                    { name: string }
                );
    ...
    const findResults = async (value) => {
        try {
            const { data } = await roomInstance.get(
                `${process.env.REACT_APP_CHAT_SERVER_URL}api/chat/room?string=${value}`
            );
            if (data.status === 'success') {
                setRooms(data.rooms);
            }
